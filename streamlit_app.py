...
class StationDatabase:
    ...
    def __init__(self, db_path: Path = STATIONS_DB_PATH):
        ...
        self._github_token = ""
        try:
            self._gist_id = st.secrets.get("GIST_ID", "")
            self._github_token = st.secrets.get("GITHUB_TOKEN", "")
        except Exception:
            pass
    ...
    def _gist_enabled(self) -> bool:
        return bool(self._gist_id and self._github_token)

    # --- Load ---
    def _load(self) -> Dict:
        # Try Gist first (persistent across cloud restarts)
        if self._gist_enabled:
            gist_data = self._load_from_gist()
            if gist_data:
                self._save_local(gist_data)
                return gist_data
    ...
    def _load_from_gist(self) -> Optional[Dict]:
        try:
            resp = requests.get(
            ...
            content = resp.json()["files"]["stations_db.json"]["content"]
...
def main():
    ...
    /* Force all nested divs inside input wrappers to be white */
    div[data-testid="stTextInput"] div,
    div[data-testid="stNumberInput"] div,
    div[data-testid="stMultiSelect"] div[data-baseweb] div,
    div[data-testid="stSelectbox"] div[data-baseweb] div {
        background-color: #ffffff !important;
    }
